		...
		<script type="text/javascript">
			...
			function init(){
				...
				function savefunc(){
					...
					http.send("xvals="+JSON.stringify(dotsX)+"&yvals="+JSON.stringify(dotsY)+"&colorvals="+JSON.stringify(dotColor)+"&sizevals="+JSON.stringify(dotSizes));
				}

				function loadfunc(){
					alert("loading");
					var http = new XMLHttpRequest();
					http.open("GET", "data2.txt", true);
					http.onreadystatechange = function(){
						alert("here");
						if(http.readyState == 4 && http.status == 200){
							var loadedText = http.responseText;
							alert(loadedText);
						}
					}
				}
